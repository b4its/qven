...
class SuperadminAktifitasKotakMBGTable extends TableWidget
{
    // Mengatur span menjadi 1 agar mengambil setengah layar (berada di kiri)
    protected int | string | array $columnSpan = 1;

    protected static ?string $heading = 'Aktivitas Khusus Kotak MBG';
    
    // Opsional: Urutan widget di dashboard
    ...
    public function table(Table $table): Table
    {
        return $table
            ->query(
                // Filter ketat hanya untuk tabel kotak_mbg
                Aktifitas::query()
                    ->where('table_name', 'kotak_mbg')
                    ->latest()
            )
